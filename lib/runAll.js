...
const validFilesOverride = files =>
 ...
 * @param {Array<string>} [options.files] - Override staged files with custom list of file paths, absolute or relative to the git root
...
module.exports = async function runAll(
  ...
  if (filesOverride && !validFilesOverride(filesOverride)) {
    throw new Error(
      'Invalid files passed! `files` should be an array of absolute file paths (string).'
    )
  }

  const files = filesOverride || (await getStagedFiles({ cwd: gitDir }))
